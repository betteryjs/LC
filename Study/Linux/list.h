...
// foo.next = &foo;
// foo.prev = &foo;
// 它表示一个带有头节点的空双向链表，其中next和prev指针指向头节点本身
...
{
    // 在 prev_node (head)  和 next_node(head->next) 之间添加元素
 ...
 */
static inline void __list_del(struct list_head *prev_node, struct list_head *next_node)
{
...
{
    // | prev_node | <--> | delete_node |  <---> | next_node |
    // next_node->prev=prev_node;
    // prev_node->next=next_node;
